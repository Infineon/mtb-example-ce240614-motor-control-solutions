...
#define NOMINAL_SPEED (16383.0f/2)
#define MAX_SPEED (16383.0f/2)
void PARAMS_Init()
{
#if defined(PC_TEST)
...
void PARAMS_InitManual()
{
    ...
    params.motor.P = 8.0f; // [#]
    params.motor.lq = 580.0E-6f; // [H]
    params.motor.ld = 580.0E-6f; // [H]
    params.motor.lam = 6.0E-4f ; // [Wb]
    params.motor.r = 6.2f; // [Ohm]
    params.motor.T_max = 0.0390f; // [Nm]
    params.motor.i_peak = 0.16f; // [A]
    params.motor.i_cont = 0.15f; // [A]
    params.motor.id_max = 0.075f; // [A]
    params.motor.v_nom = LINE_TO_PHASE(5.0f); // [Vpk-ln]
    params.motor.w_nom.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(NOMINAL_SPEED)), params.motor.P); // [Ra/sec-elec] 945Hz elec, 19.6Vdc
    params.motor.w_max.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(MAX_SPEED)), params.motor.P); // [Ra/sec-elec]
    ...
    params.sys.samp.fs0_fs1_ratio = 20U; // []
#if defined(PC_TEST)
...
#endif
    params.sys.analog.shunt.hyb_mod.adc_t_min = 3.00E-6f; // [sec]
    ...
    params.sys.rate_lim.w_cmd.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(MAX_SPEED / 4.0f)), params.motor.P); // [(Ra/sec-elec)/sec]
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    params.sys.rate_lim.i_cmd = 0.15f * 100.0f; // [A/sec]
#elif defined(CTRL_METHOD_SFO)
    params.sys.rate_lim.T_cmd = 0.390f * 100.0f; // [Nm/sec]
#endif
    // Fault Parameters:
    params.sys.faults.oc_thresh = 1.10f; // [%]
    params.sys.faults.vdc_time = 0.300f; // [sec]
    params.sys.faults.temp_ps_thresh = 70.0f; // [Celsius]
    ...
    params.sys.cmd.w_max.mech = HZ_TO_RADSEC(RPM_TO_HZ(MAX_SPEED)); //[Ra/sec-elec]
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    params.sys.cmd.i_max = 0.15f * 0.5f; // [A]
#elif defined(CTRL_METHOD_SFO)
    ...
    params.mech.inertia = 5.5E-8f; // [kg.m^2]=[(N.m)/(Ra/sec-mech).sec], =1/2*m*r^2
    params.mech.viscous = 6.0E-8f; // [kg.m^2/sec]=[(N.m)/(Ra/sec-mech)]
    params.mech.friction = 5.0E-5f; // [kg.m^2/sec^2]=[N.m]
    ...
    params.ctrl.speed.bw = HZ_TO_RADSEC(5.0f); // [Ra/sec], at least 0.5 dec below field weakening loop's bandwidth
...
#endif
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    params.ctrl.curr.bw = HZ_TO_RADSEC(1000.0f); // [Ra/sec], at least one dec below switching frequency
    ...
    params.ctrl.volt.v_min = 0.7; // [Vpk]
    params.ctrl.volt.v_to_f_ratio = 6.5E-4f ; // [Vpk/(Ra/sec-elec)]
    ...
    params.ctrl.flux_weaken.en = Dis; // [#]
    ...
    params.ctrl.six_pulse_inj.i_peak = 0.1f; // [A], this is the target value, not the actual value
    ...
    params.ctrl.high_freq_inj.i_qd_r_peak.d = 0.25f; // [A], excitation current peak (d-axis), limiting factor (larger than q-axis)
